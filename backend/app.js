...
// Classe pour gerer l'indexation et la recherche TF-IDF
class TFIDFSearchEngine {
  ...
  normalizeText(text) {
    // Normalise le texte : minuscules, suppression caractères speciaux
  ...
  preprocess(text) {
    // Applique toutes les etapes de pretraitement
  ...
  computeTF(document) {
    // Calcule la frequence des termes (TF)
  ...
  computeDocumentFrequencies() {
    // Calcule la frequence des documents pour chaque terme
    this.documentFrequencies = {};

    Object.entries(this.documents).forEach(([, document]) => {
      const tokens = new Set(this.preprocess(document));
      tokens.forEach((token) => {
        this.documentFrequencies[token] =
          (this.documentFrequencies[token] || 0) + 1;
      });
    });
  }
  ...
  computeTFIDF() {
    ...
    // D'abord, calculer la frequence des termes par document
    const totalDocs = Object.keys(this.documents).length;

    // Calculer la frequence des documents pour chaque terme
    ...
    Object.entries(this.documents).forEach(([docId, document]) => {
      ...
      Object.entries(tfDict).forEach(([token, tf]) => {
        ...
        const idf = Math.log(totalDocs / (df + 1)) + 1; // +1 pour eviter la division par zero
  ...
  cosineSimilarity(queryVector, documentVector) {
    ...
    const queryNorm = Math.sqrt(
      Object.values(queryVector).reduce(
        (sum, weight) => sum + weight * weight,
        0
      )
    );
    const docNorm = Math.sqrt(
      Object.values(documentVector).reduce(
        (sum, weight) => sum + weight * weight,
        0
      )
    );
  ...
  euclideanDistance(queryVector, documentVector) {
    ...
    // Creer un ensemble de tous les termes dans les deux vecteurs
    const allTerms = new Set([
      ...Object.keys(queryVector),
      ...Object.keys(documentVector),
    ]);

    // Calculer la somme des carres des differences
    ...
    // La distance euclidienne est la racine carree de cette somme
    const distance = Math.sqrt(sumSquaredDiff);

    // Pour la recherche, nous voulons que des valeurs plus elevees indiquent une meilleure correspondance
    // Donc nous retournons l'inverse de la distance (avec un petit epsilon pour eviter la division par zero)
    return distance; // Return the actual Euclidean distance for correct similarity calculation
  }

  search(query, similarityMethod = "cosine") {
    // Recherche les documents correspondant à la requête
    // Pretraiter la requête
    ...
    // Calculer la similarite/distance avec chaque document
    const scores = {};
    ...
    // Trier les resultats par score decroissant
...
app.post("/api/upload", upload.array("files"), (req, res) => {
  if (!req.files || req.files.length === 0) {
    return res.status(400).json({ error: "Aucun fichier trouve" });
  }

  // Vider les documents precedents
  ...
  return res.json({
    message: `${Object.keys(searchEngine.documents).length} fichiers charges`,
    documents: Object.keys(searchEngine.documents),
  });
});

// Point d'API pour indexer les documents
app.post("/api/index", (_, res) => {
